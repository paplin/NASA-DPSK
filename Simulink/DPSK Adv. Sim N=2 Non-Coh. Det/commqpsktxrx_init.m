...
SimParams.Message         = 'Good afternoon world';
SimParams.MessageLength   = strlength(SimParams.Message) + 5;             % 'Hello world 000\n'...
SimParams.NumberOfMessage = 100;                                           % Number of messages in a frame 20
...
K = 1;
A = 1/sqrt(2);
SimParams.PhaseRecoveryLoopBandwidth    = 0.01;         % 0.01 % Normalized loop bandwidth for fine frequency compensation
SimParams.PhaseRecoveryDampingFactor    = 1;            % Damping Factor for fine frequency compensation
SimParams.TimingRecoveryLoopBandwidth   = 0.01;         % 0.01 %Normalized loop bandwidth for timing recovery
...
%% Peyton Non-Coherent Parameters %%

SimParams.M = SimParams.ModulationOrder;
SimParams.phaseoffset = 0; %Do not change phase offset
SimParams.img = sqrt(-1);

for i = 1:SimParams.M
    SimParams.phaseConstel(i) = cos(2*pi*(i-1)/SimParams.M + SimParams.phaseoffset) + SimParams.img*sin(2*pi*(i-1)/SimParams.M + SimParams.phaseoffset);
end

SimParams.angleConstel = angle(SimParams.phaseConstel);

for j = 1:size(SimParams.angleConstel,2)
    if SimParams.angleConstel(j) < 0
        SimParams.angleConstel(j) = SimParams.angleConstel(j) + (2*pi);
    end
end
